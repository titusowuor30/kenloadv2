...
export default {
  ...
  data() {
    return {
      ...
      deletetransporter: false,
      viewTransporters: false,
      clickedPrintpdf: "",
      printedpdf: true,
      generatedExel: "",
      printReport: false,
      ...
      fields: [
        ...
        },
        {
          ...
          label: "Transporters Name",
          sortable: true,
        },
        {
          key: "transporter_address",
          label: "Address",
          sortable: true,
        },
        {
  ...
  watch: {
    viewTransporters(newValue) {
      ...
      const data = {
        ...
        details: `Opened transporters' list:${newValue}`,
    ...
    printReport() {
      ...
      const data = {
        ...
        useremail: curentuser,
        application: window.navigator.userAgent,
        details: `Transporters reports:${this.clickedPrintpdf},${this.generatedExel},${this.filteredValues}`,
        computer: window.localStorage.clientip,
      };
      axios
        .post(window.$http + "AuditLogs", data, { headers: window.$headers })
    ...
    addtransporter(newValue) {
      ...
      const data = {
        ...
        useremail: curentuser,
        application: window.navigator.userAgent,
        details: `Add transporter:${newValue},value:${this.transporter_name}`,
        computer: window.localStorage.clientip,
      };
      axios
        .post(window.$http + "AuditLogs", data, { headers: window.$headers })
    ...
    edittransporter(newValue) {
      ...
      const data = {
        ...
        useremail: curentuser,
        application: window.navigator.userAgent,
        details: `Edit transporter:${newValue},value:${this.transporter_name}`,
        computer: window.localStorage.clientip,
      };
      axios
        .post(window.$http + "AuditLogs", data, { headers: window.$headers })
    ...
    deletetransporter(newValue) {
      ...
      const data = {
        ...
        useremail: curentuser,
        application: window.navigator.userAgent,
        details: `Delete transporter]:${newValue},transporter id:${this.id}`,
        computer: window.localStorage.clientip,
      };
      axios
        .post(window.$http + "AuditLogs", data, { headers: window.$headers })
  ...
  methods: {
    updatearray() {
      ...
      axios
        .get(window.$http + `Transporters/search?limit=` + this.perPage, {
    ...
    getrpt(p_excel) {
      ...
      const data = this.orderData.map((row) => ({
        ID: row.id,
        Cargo_Name: row.transporter_name,
      }));
      //alert("");
      const csvRows = [];
      //get headers
    ...
    search() {
      ...
      if (this.perPage > 0) {
        ...
        fill += `limit=` + this.perPage;
      }
      alert(window.$http + `Transporters/search?` + fill);

      axios
        .get(window.$http + `Transporters/search?` + fill, {
          headers: window.$headers,
        })
        .then((response) => {
          // JSON responses are automatically parsed.
    ...
    editrec() {
      if (this.transporter_name.trim() == "") {
        Swal.fire("Please enter  Transporter Name!");
        return;
      }
      ...
      axios
        .put(window.$http + `Transporters/` + this.id, data, {
          headers: window.$headers,
        })
        .then((response) => {
          // JSON responses are automatically parsed.
          this.orderData[this.myindex].id = this.id;
          this.orderData[this.myindex].transporter_name = this.transporter_name;
          this.orderData[this.myindex].transporter_address =
            this.transporter_address;
          this.orderData[this.myindex].transporter_contact =
            this.transporter_contact;
    ...
    deleterec(index, id, transporter_name) {
      //alert(rolename);
      this.id = id;
      this.transporter_name = transporter_name;
      Swal.fire({
        title:
          "Are you sure, you want to delete " + this.transporter_name + "?",
        ...
        if (result.value) {
          axios
            .delete(window.$http + `Transporters/` + this.id, {
              headers: window.$headers,
            })
            .then((response) => {
              // JSON responses are automatically parsed.
              console.log(response.data);
              this.deletetransporter = true;
    ...
    clearvalues() {
      this.name = "";
      this.editmode = false;
      this.modaltitle = "Add Transporters";
    },
  },
};
</script>
                          ...
                          >
                            <template v-slot:cell(screens)="data">
                              <a
                                ...
                                v-for="screen in data.item.transporter_name"
                                :key="screen"
                                >{{ screen }} <br
                              /></a>
                            </template>
                            <template v-slot:cell(address)="data">
                              <a
                                href="javascript: void(0);"
                                class="text-dark fw-bold"
                                >{{ data.item.transporter_address }}</a
                              >
                            </template>
                            <template v-slot:cell(phone)="data">
                              <a
                                href="javascript: void(0);"
                                class="text-dark fw-bold"
                                >{{ data.item.transporter_contact }}</a
                              >
                            </template>
                            <!----
                                  ...
                                  <a
                                    ...
                                    @click="
                                      edit(
                                        ...
                                        data.item.transporter_address,
                                        data.item.transporter_contact
                                      )
                                    "
                                  >
