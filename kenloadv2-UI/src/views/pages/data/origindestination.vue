...
export default {
  ...
  data() {
    return {
      ...
      id: "",
      addOriginsDest: false,
      editOriginsDest: false,
      deleteOriginsDest: false,
      viewOriginDest: false,
      clickedPrintpdf: "",
      printedpdf: true,
      generatedExel: "",
      printReport: false,
  ...
  watch: {
    viewOriginDest(newValue) {
      ...
      const data = {
        ...
        details: `Opened Origins & Destinations list:${newValue}`,
    ...
    printReport() {
      ...
      const data = {
        ...
        useremail: curentuser,
        application: window.navigator.userAgent,
        details: `Origin & Destination reports:${this.clickedPrintpdf},${this.generatedExel},${this.filteredValues}`,
        computer: window.localStorage.clientip,
      };
      axios
        .post(window.$http + "AuditLogs", data, { headers: window.$headers })
    ...
    addOriginsDest(newValue) {
      ...
      const data = {
        ...
        useremail: curentuser,
        application: window.navigator.userAgent,
        details: `Add OriginsDest:${newValue},value:${this.origin_name}`,
        computer: window.localStorage.clientip,
      };
      axios
        .post(window.$http + "AuditLogs", data, { headers: window.$headers })
    ...
    editOriginsDest(newValue) {
      ...
      const data = {
        ...
        useremail: curentuser,
        application: window.navigator.userAgent,
        details: `Edit OriginsDest:${newValue},value:${this.origin_name}`,
        computer: window.localStorage.clientip,
      };
      axios
        .post(window.$http + "AuditLogs", data, { headers: window.$headers })
    ...
    deleteOriginsDest(newValue) {
      ...
      const data = {
        ...
        useremail: curentuser,
        application: window.navigator.userAgent,
        details: `Delete OriginsDest:${newValue},OriginsDest id:${this.id}`,
        computer: window.localStorage.clientip,
      };
      axios
        .post(window.$http + "AuditLogs", data, { headers: window.$headers })
  ...
  methods: {
    updatearray() {
      ...
      axios
        .get(window.$http + `Origins`, { headers: window.$headers })
        .then((response) => {
    ...
    editrec() {
      ...
      axios
        .put(window.$http + `Origins/` + this.id, data, {
          headers: window.$headers,
        })
        .then((response) => {
          // JSON responses are automatically parsed.
          this.orderData[this.myindex].id = this.id;
          this.orderData[this.myindex].origin_name = this.origin_name;
          this.orderData[this.myindex].active = this.active;
          console.log(response.data);
          this.editOriginsDest = true;
    ...
    position() {
      if (this.origin_name.trim() == "") {
        Swal.fire("Please enter  Origing / Destination!");
        return;
      }
    ...
    deleterec(index, id, origin_name) {
      //alert(rolename);
      this.id = id;
      this.origin_name = origin_name;
      Swal.fire({
        title: "Are you sure, you want to delete " + this.origin_name + "?",
        ...
        if (result.value) {
          axios
            .delete(window.$http + `Origins/` + this.id, {
              headers: window.$headers,
            })
            .then((response) => {
              // JSON responses are automatically parsed.
              console.log(response.data);
              this.deleteOriginsDest = true;
              this.orderData.splice(index, 1);
              Swal.fire(
                this.origin_name + " Deleted!",
                          ...
                          >
                            <template v-slot:cell(screens)="data">
                              <a
                                ...
                                v-for="screen in data.item.origin_name"
                                :key="screen"
                                >{{ screen }} <br
                              /></a>
                            </template>
